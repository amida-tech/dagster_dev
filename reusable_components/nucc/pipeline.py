...
def create_nucc_pipeline(config: dict):
    ...
    @asset(
        ...
        description=f"Copy {subject_area} CSV files to load directory with version number removal",
    ...
    def copy_files_to_load(context: AssetExecutionContext, monitor_result) -> MaterializeResult:
        """Copy CSV files from stage to load directory with version number removal"""
        ...
        try:
            ...
            for path in fs_client.get_paths(path=config["stage_directory"]):
                ...
                if (any(filename.lower().startswith(p.lower()) for p in config["file_criteria"]["prefix"]["pattern"]) and 
                    ...
                    clean_name = _remove_timestamp(filename)
    ...
    def execute_rules_asset(context: AssetExecutionContext, audit_batch_id: int, iceberg_result):
        ...
        try:
            ...
            directories_to_clean = [
                {
                    "pipeline_name": "NUCC",
                    "stage_directory": NUCC_CONFIG["stage_directory"],
                    "load_directory": NUCC_CONFIG["load_directory"]
                }
            ]
...
def _remove_timestamp(filename):
    """Remove timestamp AND version numbers from filename - keep everything before the timestamp/version"""
...
def upload_large_file_to_adls(context, adls_client, file_content, file_name, config, target_directory):
    ...
    try:
        ...
        if file_size_mb <= 50:
            context.log.info("Trying upload...")
            max_retries = 3
            
            for attempt in range(max_retries):
                try:
                    file_client.upload_data(file_content, overwrite=True)
                    context.log.info("upload succeeded")
                    return True
                except Exception as error:
                    context.log.warning(f"upload attempt {attempt + 1} failed: {str(error)[:100]}")
