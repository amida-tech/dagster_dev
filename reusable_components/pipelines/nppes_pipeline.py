...
from reusable_components.etl.bronze_to_silver import transform_bronze_to_silver_npi
from reusable_components.etl.silver_to_gold import transform_silver_to_gold as silver_to_gold_function
...
def create_pipeline(config: dict):
    ...
    @asset(
        ...
        description=f"Transform bronze {subject_area} data into 5 silver NPI tables",
        required_resource_keys={"snowflake_snowpark"},
        ins={
            "audit_batch_id": AssetIn(f"start_dq_audit_run_{subject_area}"),
    ...
    def transform_bronze_to_silver(context: AssetExecutionContext, audit_batch_id: int, iceberg_result) -> MaterializeResult:
        return transform_bronze_to_silver_npi(
            context=context,
            snowpark_session=context.resources.snowflake_snowpark,
            audit_batch_id=audit_batch_id,
            config=config
    ...
    @asset(
        ...
        description="Transform silver NPI data to gold using MD5 hash comparison",
        required_resource_keys={"snowflake_snowpark"},
        ins={
            "audit_batch_id": AssetIn(f"start_dq_audit_run_{subject_area}"),
    ...
    def silver_to_gold_asset(context: AssetExecutionContext, audit_batch_id: int, silver_result) -> MaterializeResult:
        return silver_to_gold_function(
            context=context,
            snowpark_session=context.resources.snowflake_snowpark,
            audit_batch_id=audit_batch_id,
            config=config
